...
function selectObject(obj, el) {
    ...
    var result = "<h2>"+ "Overview for:  " + obj.name;
    result += "<h2> SpanId:<p>" + obj.id + "ms";
    result += "<h2> TraceId:<p>" + obj.traceId + "ms";
    result += "<h2> Duration:<p>" + (obj.duration/1000) + "ms";
    if (obj.annotations != undefined) {
    	result += "<h3> Annotations:";
    	for (var i in obj.annotations) {
    		result += "<p>" + JSON.stringify(obj.annotations[i]);
    	}
    }
    if (obj.binaryAnnotations != undefined) {
    	result += "<h3> Binary Anotations:";
    	for (var i in obj.binaryAnnotations) {
    		result += "<p>" + JSON.stringify(obj.binaryAnnotations[i]);
    	}
    }
    
    $('#docs').html("<p>"+ result);
//    $('#docs').html("<p>"+ JSON.stringify(obj));
    ...
    if (obj) {
        if (obj !== highlighted) {
            graph.node.classed('inactive', function(d) {
                return (obj !== d &&d.parentId != undefined
                     && d.parentId.indexOf(obj.name) == -1);
            });
